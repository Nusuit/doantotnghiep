...
import { CURRENT_USER as MOCK_USER } from '@/data/mockData';
...
export default function LeaderboardPage() {
    ...
    const displayedUsers = useMemo(() => {
        if (!searchQuery.trim()) {
            return richLeaderboard.slice(0, 15); // Show Top 15
        }
    ...
    // Mock Rank Logic for "Me" Row
    const myRankData = useMemo(() => {
        // In a real app, we would fetch the user's specific rank from API
        // Here we will simulate it for demonstration
        const isLoggedIn = !!user;
        const displayName = user?.name || MOCK_USER.name;

        // Let's pretend the user is Rank #42 for demo purposes if not found in top list
        const userInTop = richLeaderboard.find(u => u.user.name === displayName);

        if (userInTop) return userInTop;

        // Construct a mock "Me" entry if not in top list
        return {
            rank: 42,
            user: { name: displayName, handle: user?.email ? `@${user.email.split('@')[0]}` : MOCK_USER.handle },
            points: 1250,
            articles: 5,
            reviews: 12,
            style: 'bg-indigo-50 dark:bg-indigo-900/20 border-indigo-200 dark:border-indigo-500/30',
    ...
    }, [user, richLeaderboard]);

    return (
        <div className="max-w-5xl mx-auto space-y-8 pb-32"> {/* Increased padding-bottom */}
            ...
            <div className="bg-white/50 dark:bg-dark-surface/50 backdrop-blur-xl border border-gray-200 dark:border-gray-700 rounded-3xl p-8 shadow-xl overflow-visible relative min-h-[600px]">
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto">
                        ...
                        <div className="flex p-1 bg-gray-100 dark:bg-black/40 rounded-lg border border-gray-200 dark:border-gray-800">
                            <button
                                ...
                                className={`px-3 py-1.5 rounded-md text-[10px] font-bold uppercase tracking-widest transition-all duration-300 ${timeframe === 'cycle'
                            ...
                            <button
                                ...
                                className={`px-3 py-1.5 rounded-md text-[10px] font-bold uppercase tracking-widest transition-all duration-300 ${timeframe === 'allTime'
                                    ? 'bg-white dark:bg-gray-700 text-black dark:text-white shadow-sm'
                                    : 'text-gray-500 hover:text-gray-900 dark:hover:text-gray-300'
                                    }`}
                            >
                ...
                <div className="space-y-3 relative">
                    ...
                    {displayedUsers.length > 0 ? displayedUsers.map((user) => (
                        <div
                            ...
                            className={`relative grid grid-cols-12 items-center px-4 py-3 rounded-xl border transition-all duration-300 ease-out cursor-default ${user.style} ${hoveredRank === user.rank
                                ? 'scale-[1.02] shadow-2xl z-20 border-opacity-100 ring-1 ring-white/20'
                        ...
                        >
                            ...
                            <div className="col-span-5 sm:col-span-5 flex items-center gap-3">
                                <div className={`w-10 h-10 rounded-full flex items-center justify-center text-xs font-bold font-mono tracking-wider flex-shrink-0 ${user.rank <= 3
                                ...
                                <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2 min-w-0">
                                    <span className="font-bold text-gray-900 dark:text-white truncate">{user.user.name}</span>
                                    <span className={`text-[9px] sm:text-[10px] px-2 py-0.5 rounded-full border truncate flex items-center gap-1 ${user.badge === 'Sage' ? 'bg-red-500/10 text-red-500 border-red-500/20' :
                            ...
                            <div className="col-span-2 text-right">
                                <span className={`font-bold text-xs px-2 py-1 rounded-lg ${user.rank <= 3 ? 'text-green-600 dark:text-green-400 bg-green-500/10' :
                    ...
                    {/* --- STICKY "ME" ROW (Integrated) --- */}
                    <div className="sticky bottom-0 z-50 mt-2">
                        {/* Visual Separator */}
                        <div className="absolute -top-4 left-0 right-0 h-4 bg-gradient-to-t from-gray-100/90 to-transparent dark:from-[#0B0E14]/90 pointer-events-none"></div>

                        <div className="relative grid grid-cols-12 items-center px-4 py-3 rounded-xl border border-orange-200 dark:border-orange-500/30 bg-orange-50 dark:bg-orange-900/20 shadow-[0_-4px_20px_-5px_rgba(249,115,22,0.15)] backdrop-blur-md">

                            {/* "YOU" Badge */}
                            <div className="absolute -top-2 left-1/2 -translate-x-1/2 bg-orange-500 text-white text-[9px] font-bold px-2 py-0.5 rounded-full shadow-sm z-10">YOU</div>

                            {/* Rank */}
                            <div className="col-span-2 sm:col-span-1 flex justify-center">
                                <span className="font-mono font-bold text-orange-600 dark:text-orange-400">#{myRankData.rank}</span>
                            </div>

                            {/* User */}
                            <div className="col-span-5 sm:col-span-5 flex items-center gap-3">
                                <div className="w-10 h-10 rounded-full bg-orange-100 dark:bg-orange-900/50 text-orange-600 dark:text-orange-300 flex items-center justify-center text-xs font-bold font-mono tracking-wider flex-shrink-0 border border-orange-200 dark:border-orange-500/20">
                                    {getInitials(myRankData.user.name)}
                                </div>
                                <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2 min-w-0">
                                    <span className="font-bold text-gray-900 dark:text-white truncate">{myRankData.user.name}</span>
                                    <span className="text-[9px] sm:text-[10px] px-2 py-0.5 rounded-full border border-orange-200 dark:border-orange-500/20 bg-white/50 dark:bg-black/20 text-orange-600 dark:text-orange-400 truncate flex items-center gap-1">
                                        {myRankData.badge}
                                    </span>
                                </div>
                            </div>
                            ...
                            {/* Stats */}
                            <div className="col-span-3 sm:col-span-2 text-right sm:text-center font-mono font-bold text-gray-800 dark:text-orange-100">{myRankData.points.toLocaleString()}</div>
                            <div className="col-span-2 sm:col-span-2 text-center text-sm text-gray-600 dark:text-gray-400 hidden sm:block"><span className="flex items-center justify-center gap-1"><span className="font-bold">{myRankData.articles.toLocaleString()}</span> / {myRankData.reviews.toLocaleString()}</span></div>

                            {/* Reward */}
                            <div className="col-span-2 text-right">
                                <span className="font-bold text-xs px-2 py-1 rounded-lg text-orange-700 dark:text-orange-300 bg-orange-100 dark:bg-orange-500/10 border border-orange-200 dark:border-orange-500/20">
                                    {myRankData.reward}
                                </span>
                            </div>
                        </div>
