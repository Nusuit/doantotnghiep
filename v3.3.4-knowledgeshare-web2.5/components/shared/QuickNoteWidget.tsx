...
export const QuickNoteWidget = () => {
    const { isQuickNoteVisible, setQuickNoteVisible } = useWidget();
    ...
    }, [isDragging, isHoveringDelete, position.x, setQuickNoteVisible]);
    ...
    return (
        <>
            ...
            >
                ...
                >
                    {/* Header */}
                    <div className="flex items-center justify-between px-4 py-3 bg-[#2970FF] text-white shadow-md z-10 shrink-0">
                        {viewMode === 'editor' ? (
                    ...
                    <div className="flex-1 relative bg-gray-50 dark:bg-[#0B0E14] overflow-hidden">
                        ...
                        {viewMode === 'editor' && (
                            <textarea ref={textareaRef} value={activeNote?.content || ''} onChange={(e) => handleUpdateContent(e.target.value)} placeholder="Type something..." className="w-full h-full bg-white dark:bg-[#0B0E14] text-sm text-gray-800 dark:text-gray-200 p-4 resize-none outline-none leading-relaxed custom-scrollbar" />
                        )}
                    </div>
                </div>
                ...
                <div 
                    ...
                    } ${isOpen ? 'bg-gray-800 rotate-90' : 'bg-[#2970FF]'}`}
                >
                    {/* Icon */}
                    ...
                    {!isOpen && notes.length > 0 && (
                        <div className="absolute -top-1 -right-1 bg-red-500 text-white text-[10px] font-bold w-5 h-5 flex items-center justify-center rounded-full border-2 border-[#2970FF]">
                            {notes.length}
                        </div>
                    )}
                </div>
