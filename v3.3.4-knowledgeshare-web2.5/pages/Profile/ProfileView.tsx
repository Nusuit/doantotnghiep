...
export const ProfileView = () => {
    ...
    const handleWriteReservation = (reservation: any) => {
        navigate('/app/create', { state: { reservation } });
    };
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-6">
            ...
            <div className="relative rounded-3xl bg-white dark:bg-white/5 backdrop-blur-md border border-gray-200 dark:border-white/10 shadow-xl dark:shadow-[0_0_20px_-5px_rgba(79,70,229,0.1)] overflow-hidden transition-all duration-500">
                ...
                <div className="p-8 flex flex-col items-start gap-6 relative z-10">
                    <div className="flex flex-col md:flex-row gap-8 w-full items-start">
                        ...
                        <div className="relative flex-shrink-0">
                                <div className="w-24 h-24 rounded-full bg-gradient-to-br from-gray-800 to-black border border-white/10 flex items-center justify-center shadow-xl">
                                <span className="text-3xl font-serif font-bold text-white tracking-wider">{getInitials(CURRENT_USER.name)}</span>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center gap-4 text-sm mb-3">
                                        <div className="flex items-center gap-1 group cursor-pointer">
                                            <span className="font-bold text-gray-900 dark:text-white group-hover:text-primary transition-colors">{formatNumber(CURRENT_USER.followers)}</span>
                                            <span className="text-gray-500">Followers</span>
                                        </div>
                                        <div className="w-1 h-1 rounded-full bg-gray-300 dark:bg-gray-700"></div>
                                        <div className="flex items-center gap-1 group cursor-pointer">
                                            <span className="font-bold text-gray-900 dark:text-white group-hover:text-primary transition-colors">{formatNumber(CURRENT_USER.following)}</span>
                ...
                <div 
                    className={`bg-gray-50 dark:bg-black/20 border-t border-gray-200 dark:border-white/5 transition-all duration-500 ease-in-out overflow-hidden ${
                        showSuggestions ? 'max-h-[300px] opacity-100' : 'max-h-0 opacity-0'
                    }`}
                >
                    <div className="p-6">
            ...
            {/* 2. ASSETS & STATS ROW (Simplified & Compact) */}
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                
                {/* Card 1: Utility */}
                <div 
                    onClick={() => setShowWalletAnalytics(true)}
                    className="bg-white dark:bg-white/5 backdrop-blur-md border border-gray-200 dark:border-white/10 rounded-2xl p-4 flex items-center gap-4 hover:border-blue-500/30 cursor-pointer transition-all shadow-sm group"
                >
                    ...
                    <div>
                        ...
                        <div className="flex items-baseline gap-1">
                            <span className="text-xl font-mono font-bold text-gray-900 dark:text-white">1,240</span>
                ...
                <div 
                    ...
                    className="bg-white dark:bg-white/5 backdrop-blur-md border border-gray-200 dark:border-white/10 rounded-2xl p-4 flex items-center gap-4 hover:border-purple-500/30 cursor-pointer transition-all shadow-sm group"
                >
                    ...
                    <div>
                        ...
                        <div className="flex items-baseline gap-1">
                            <span className="text-xl font-mono font-bold text-gray-900 dark:text-white">1,500</span>
                ...
                {/* Card 3: Reputation */}
                <div className="bg-white dark:bg-white/5 backdrop-blur-md border border-gray-200 dark:border-white/10 rounded-2xl p-4 flex items-center gap-4 shadow-sm group">
                    ...
                    <div>
                        ...
                        <div className="flex items-baseline gap-1">
                            <span className="text-xl font-mono font-bold text-gray-900 dark:text-white">850</span>
            ...
            <div className="bg-white dark:bg-white/5 backdrop-blur-md border border-gray-200 dark:border-white/10 rounded-3xl overflow-hidden">
                    <div className="flex border-b border-gray-200 dark:border-gray-700">
                        {['contributions', 'suggestions', 'votes', 'proposals', 'reservations'].map((tab) => (
                            <button 
                                key={tab}
                                onClick={() => setActiveTab(tab as any)}
                                className={`flex-1 py-4 text-sm font-bold uppercase tracking-wider hover:bg-gray-50 dark:hover:bg-white/5 transition-colors ${
                                    activeTab === tab 
                                            ...
                                            <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-white/5 transition-colors group">
                                                ...
                                                <td className="px-4 py-4 text-center">
                                                    <span className={`px-2 py-1 rounded text-xs font-bold ${
                                                        item.status === 'Approved' || item.status === 'Active' || item.status === 'Passed' ? 'bg-green-100 dark:bg-green-500/20 text-green-600 dark:text-green-400' :
                                                ...
                                                <td className="px-2 py-4 text-right relative">
                                                    ...
                                                    {openMenuRowId === item.id && (
                                                        <div className="absolute right-8 top-8 w-56 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl shadow-2xl z-50 overflow-hidden animate-fade-in-up">
                                                            ...
                                                            {item.type === 'proposal' && (
                                                                <>
                                                                    <button onClick={() => navigate('/app/governance')} className="w-full text-left px-4 py-3 text-xs font-bold text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-white/5 flex items-center gap-2 transition-colors">
