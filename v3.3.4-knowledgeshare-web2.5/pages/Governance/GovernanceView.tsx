...
const InfoTooltip = ({ title, content }: { title?: string, content: string }) => (
    <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-3 w-56 p-3 bg-[#1A1D24] border border-gray-700 rounded-xl shadow-[0_10px_30px_-5px_rgba(0,0,0,0.8)] opacity-0 group-hover:opacity-100 transition-all duration-200 pointer-events-none z-50 text-center transform translate-y-2 group-hover:translate-y-0">
                ...
                <div className="flex flex-col md:flex-row justify-between items-end gap-4">
                    ...
                    <div className="flex gap-2">
                        <button 
                            ...
                            className="p-3 rounded-xl border border-gray-200 dark:border-gray-700 text-gray-500 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-white/5 transition-all group relative"
                            title="View Leaderboard"
                        >
                            <span className="material-symbols-outlined text-[20px]">leaderboard</span>
                            {/* Tooltip */}
                            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-max px-2 py-1 bg-black text-white text-[10px] font-bold rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
                                Leaderboard
                            </div>
                        </button>
                        <button 
                            onClick={() => setShowCreateModal(true)}
                            className="p-3 rounded-xl bg-black dark:bg-white text-white dark:text-black hover:opacity-80 transition-all shadow-md group relative"
                            title="New Proposal"
                        >
                    ...
                    <div className="group bg-white dark:bg-dark-surface border border-gray-200 dark:border-gray-700 p-6 rounded-2xl relative overflow-visible">
                        ...
                        <div className="relative z-10">
                            ...
                            <div className="mt-2 text-xs text-gray-500 flex items-center gap-1">
                                Next epoch closes in <span className="text-white font-bold">14h 20m</span>
                            </div>
                        </div>
                    </div>
                            ...
                            <div className="flex flex-wrap justify-between items-start mb-4 gap-4">
                                <div className="flex items-center gap-3">
                                    <span className={`px-2.5 py-1 rounded text-[10px] font-bold uppercase tracking-wider border ${
                                        prop.type === 'Treasury' ? 'bg-blue-500/10 text-blue-500 border-blue-500/20' :
                                    ...
                                    <span className="text-xs text-gray-400 flex items-center gap-1">
                                        by <span className="text-white hover:underline cursor-pointer">{prop.author}</span>
                                    </span>
                                </div>
                                <div className={`flex items-center gap-2 text-sm font-bold ${
                                    prop.status === 'Active' ? 'text-green-500' : 
                                    ...
                                    {prop.status === 'Active' ? (
                                        <>
                                            ...
                                            <button 
                                                ...
                                                disabled={hasVoted}
                                                className={`flex-1 sm:flex-none px-6 py-2 rounded-lg font-bold text-sm transition-all shadow-sm ${
                                                    hasVoted 
                        ...
                        <div className="space-y-1">
                            {['All', 'Active', 'Passed', 'Defeated'].map((status) => (
                                <button
                                    ...
                                    className={`w-full text-left px-4 py-2.5 rounded-lg text-sm font-bold transition-all flex justify-between items-center ${
                                        filter === status 
