...
router.put("/profile", authenticateJWT, async (req: any, res: Response) => {
  try {
    ...
    if (existingProfile.length === 0) {
      ...
      await pool.execute(
        `INSERT INTO user_profiles (
          user_id, display_name, first_name, last_name, birth_date, 
          gender, phone_number, country, address, bio, 
    ...
    // Also update full_name in users table if it exists
    try {
      if (displayName) {
        await pool.execute(
      ...
      // Ignore if full_name column doesn't exist in users table
      console.log("Note: full_name column not found in users table");
    }
